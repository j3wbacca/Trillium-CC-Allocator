...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:fn="urn:trillium" version="1.0">
  ...
  <xsl:key name="group-by-cardholder" match="Allocations/Allocation" use="Cardholder_x0020_Name" />
  <xsl:template match="/">
  <html>
    <head>
      <style type="text/css">
  ...
  <body>
    <div align="center" class="divInvoice">
      <span class="large">TRANSACTIONS BY CARDHOLDER</span>
    </div>
    <hr></hr>
    <table align="center" class="reportTable">
      <tr class="headerRow">
        <th class="wsmall">Process Date</th>
      ...
      <xsl:for-each select="Allocations/Allocation[count(. | key('group-by-cardholder', Cardholder_x0020_Name)[1]) = 1]">
        <tr class="accountRow">
          <td colspan="6">
            <xsl:value-of select="Cardholder_x0020_Name" />
          </td>
        </tr>
        <xsl:for-each select="key('group-by-cardholder', Cardholder_x0020_Name)">
        ...
        <tr>
          <td colspan="4" class="amountTotal">Cardholder Total:</td>
          <td>
            <span class="amountTotal">
              <xsl:value-of select="format-number(sum(key('group-by-cardholder', Cardholder_x0020_Name)/Amount), '$###,###,##0.00')"/>
            </span>
          </td>
        </tr>
      </xsl:for-each>
